...
<div class="flex flex-col items-center text-center justify-center bg-[var(--lightbackground)] pt-10 pb-14">
    ...
    <div class="flex gap-1 justify-center w-[90%] md:w-[65%] lg:w-[50%] xl:w-[30%] px-8 pt-4">
        <input class="w-full rounded-lg px-4 border-transparent focus:outline-none focus:ring-0 focus:border-[var(--accent)]" type="text" bind:value={email} placeholder="name@gmail.com" onkeydown={(event) => { if (event.key === 'Enter') { onSubmit() } }}>
        <button class="whitebutton px-6 text-white" onclick={onSubmit} >
