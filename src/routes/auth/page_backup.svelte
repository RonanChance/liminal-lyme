<script>
    ...
    async function login_user(form) {
        console.log(oauth_selection)
          try {
              console.log("Starting login")
              const data = await pb.collection("users").authWithOAuth2({ provider: oauth_selection, urlCallback: (url) => { w.location.href = url }, })
...
<div class="divbg" in:fly|global={{y: 30, delay: 50, duration: 1000 }}>
  <h1>Open<span class="highlighted-word" style="font-weight:500">RXN</span> Login</h1>
  
  <form method="post" on:submit|preventDefault={(e) => login_user(e.currentTarget)}>
  <input name="token" type="hidden" />
      <div class="buttondiv">
        <button class="gsi-material-button" on:click={() => {oauth_selection = 'google'}}>
